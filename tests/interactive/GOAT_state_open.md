...
H_result = [Hd] + [[Hc, np.array(res_goat_time.optimized_controls[0])]]
evolution_time = qt.mesolve(H_result, initial_state, times)
...
H_result = [Hd, [Hc, np.array(res_goat_global.optimized_controls[0])]]
evolution_global = qt.mesolve(H_result, initial_state, times)
...
plt.plot(times, [qt.fidelity(state, target_state) for state in evolution_guess.states], 'k--', label="Goat")
